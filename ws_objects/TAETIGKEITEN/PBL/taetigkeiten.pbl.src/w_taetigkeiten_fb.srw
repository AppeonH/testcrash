$PBExportHeader$w_taetigkeiten_fb.srw
$PBExportComments$Tätigkeitenerfassung für Fachbereiche
forward
global type w_taetigkeiten_fb from w_taetigkeiten_basis
end type
type uo_zeitraum from u_auswahl_zeitraum within w_taetigkeiten_fb
end type
type cb_wechsel from commandbutton within w_taetigkeiten_fb
end type
type cb_cancel from commandbutton within w_taetigkeiten_fb
end type
type cb_ok from commandbutton within w_taetigkeiten_fb
end type
type dw_tk_single from datawindow within w_taetigkeiten_fb
end type
type dw_tk_liste_cross from datawindow within w_taetigkeiten_fb
end type
type dw_tk_liste_grid from datawindow within w_taetigkeiten_fb
end type
type cb_new_umlage from commandbutton within w_taetigkeiten_fb
end type
type cb_delete from commandbutton within w_taetigkeiten_fb
end type
end forward
...
global w_taetigkeiten_fb w_taetigkeiten_fb
...
this.Control[iCurrent+5]=this.dw_tk_single
this.Control[iCurrent+6]=this.dw_tk_liste_cross
this.Control[iCurrent+7]=this.dw_tk_liste_grid
this.Control[iCurrent+8]=this.cb_new_umlage
this.Control[iCurrent+9]=this.cb_delete
end on

on w_taetigkeiten_fb.destroy
call super::destroy
...
string ls_maxpos, ls_tk, ls_values
...
// darf die externe KST und TAB-Systems bebucht werden?
select edv_tk
into :ls_tk
...
IF ls_tk <> "e" THEN
	ls_values = dw_tk_single.Describe ("schluessel.values")
	
	// aus den Wertepaaren den Schlüssel "e" (Extern) entfernen
	ls_values = of_remove_keyvalue (ls_values, "e")
	
	// aus den Wertepaaren den Schlüssel "t" (TAB-Systems) entfernen
	ls_values = of_remove_keyvalue (ls_values, "t")
	
	dw_tk_single.Modify ("schluessel.values='" + ls_values + "'")
END IF

// Default-Schlüssel festlegen (nachdem ggf. Schlüssel wegen Berechtigung entfernt wurden)
...
type dw_zeitdaten from w_taetigkeiten_basis`dw_zeitdaten within w_taetigkeiten_fb
integer x = 2917
integer y = 0
integer height = 1936
end type
...
type dw_info from w_taetigkeiten_basis`dw_info within w_taetigkeiten_fb
integer x = 50
integer width = 2825
end type

type ds_schluessel from w_taetigkeiten_basis`ds_schluessel within w_taetigkeiten_fb
end type
...
integer taborder = 40
string title = "none"
string dataobject = "d_tk_single_fb"
...
if dwo.name = "u_p" or dwo.name = "u_e" or dwo.name = "u_t" or dwo.name = "u_b" then
	// Umlage für FB nur für Projekt und Extern auswählbar
...
elseif dwo.name = "schluessel" then
	...
	ELSE
		...
		// DDDWs setzen
		IF data = "p" THEN
			ls_mod = "zusatz_dddw.DDDW.Name='d_dddw_zusatz_p'" 
		END IF
		IF ls_mod <> "" THEN
			Modify (ls_mod)
			IF getChild ("zusatz_dddw", child_dddw) > 0 THEN
				child_dddw.SetTransObject (SQLCA)
				child_dddw.Retrieve (gs_modus, GetItemString (row, "u_p"))
			END IF
		END IF
	END IF
...
IF GetChild ("zusatz_dddw", ldwc_current) = 1 THEN
	ldwc_current.SetTransObject (SQLCA)
	ldwc_current.Retrieve (gs_modus, "")
END IF
