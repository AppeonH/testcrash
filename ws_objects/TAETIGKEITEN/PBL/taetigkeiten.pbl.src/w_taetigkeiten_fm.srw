$PBExportHeader$w_taetigkeiten_fm.srw
$PBExportComments$Tätigkeitenerfassung für Fachbereich Forderungsmanagement
forward
global type w_taetigkeiten_fm from w_taetigkeiten_basis
end type
type uo_zeitraum from u_auswahl_zeitraum within w_taetigkeiten_fm
end type
type cb_wechsel from commandbutton within w_taetigkeiten_fm
end type
type cb_ok from commandbutton within w_taetigkeiten_fm
end type
type dw_tk_single from datawindow within w_taetigkeiten_fm
end type
type dw_tk_liste_cross from datawindow within w_taetigkeiten_fm
end type
type dw_tk_liste_grid from datawindow within w_taetigkeiten_fm
end type
type cb_new_umlage from commandbutton within w_taetigkeiten_fm
end type
type cb_delete from commandbutton within w_taetigkeiten_fm
end type
end forward
...
global w_taetigkeiten_fm w_taetigkeiten_fm
...
type dw_zeitdaten from w_taetigkeiten_basis`dw_zeitdaten within w_taetigkeiten_fm
integer x = 3049
integer y = 0
integer height = 2116
end type
...
type dw_info from w_taetigkeiten_basis`dw_info within w_taetigkeiten_fm
integer x = 50
integer width = 2967
end type

type ds_schluessel from w_taetigkeiten_basis`ds_schluessel within w_taetigkeiten_fm
end type
...
integer taborder = 40
string title = "none"
string dataobject = "d_tk_single_fm"
...
if dwo.name = "u_p" then
	...
	// Umlage für FM nur für Projekt, Beratung oder Kostenträger auswählbar
...
IF GetChild ("zusatz", ldwc_current) = 1 THEN
	ldwc_current.SetTransObject (SQLCA)
	ldwc_current.Retrieve (1)
END IF

IF GetChild ("zusatz1", ldwc_current) = 1 THEN
	ldwc_current.SetTransObject (SQLCA)
	ldwc_current.Retrieve (2)
END IF
...
string title = "none"
string dataobject = "d_tk_liste_fm"
boolean hscrollbar = true
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
